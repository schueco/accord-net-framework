                ...
                for (int y = 0; y < height; y++)
                {
                    for (int x = 0; x < width; x++, dst++)
                    {
                        ...
                        for (int i = pixel.Length - 1; i >= 0; i--, src++)
                            pixel[i] = Accord.Math.Vector.Scale(*src, (byte)0, (byte)255, Min, Max);
                    }
                    src += offset;
                }
            }
                ...
                if (input.PixelFormat == PixelFormat.Format16bppGrayScale)
                {
                    ...
                    for (int y = 0; y < height; y++)
                    {
                        for (int x = 0; x < width; x++, dst++, src++)
                            output[dst] = Accord.Math.Vector.Scale(*src, 0, 65535, Min, Max);
                ...
                else
                {
                    ...
                    for (int y = 0; y < height; y++)
                    {
                        for (int x = 0; x < width; x++, dst++, src += pixelSize)
                            output[dst] = Accord.Math.Vector.Scale(*src, (byte)0, (byte)255, Min, Max);
